...
CURRENT_JUMP_VERSION="$($JUMP_BINARY version 2>/dev/null)"
CURRENT_GIT_VERSION=$(cat "$CURRENT_DIR/VERSION" 2>/dev/null)

SKIP_WIZARD=$(tmux show-option -gqv @jump-skip-wizard)
SKIP_WIZARD=${SKIP_WIZARD:-0}

if [[ "$SKIP_WIZARD" = "0" && -n "$CURRENT_GIT_VERSION" && "$CURRENT_JUMP_VERSION" != "$CURRENT_GIT_VERSION" ]]; then
	tmux run-shell -b "JUMP_UPDATE=1 bash $CURRENT_DIR/install-wizard.sh"
fi

key=$(tmux show-option -gqv @jump-key)
[[ -z "$key" ]] && key=j

tmux bind-key -N "Jump to visible text in copy mode" "$key" \
	run-shell -b "JUMP_BINARY='$JUMP_BINARY' $CURRENT_DIR/tmux-jump.sh"
